...
int cmp(char* s, char* s_) {
    ...
    if (strncmp(s1, s2, MAX_SIZE) < 0) return 0;
...
int existsN(char* name) {
    for (book* w = head; w; w = w->next) if (cmp(w->bookName, name) == 1) return 1;
    return 0;
}
int existsB(char* name) {
    for (book* w = head; w; w = w->next) if (cmp(w->borrower, name) == 1) return 1;
    return 0;
}
void fixB(book* node) {
    fixN(node->bookName);
